...
static const size_t VOL_SIZE = 0x8000 * 0x100;
...
static uint8_t rom_buff[VOL_SIZE];
static uint8_t nor_buff[NOR_SIZE];
static uint8_t temp_buff[0x8000];

/* ROM cache */
static FILE* f_rom = NULL;
static uint8_t vol_idx;	/* current vol idx of rom_buff */

static uint8_t* rom_volume[0x100];
...
/* vol{0,1,2} */
void LoadVolume(uint8_t vol){
	if (vol != vol_idx) {
		printf("Volume Need: %d, Current: %d \n", vol, vol_idx);
		fseek(f_rom, VOL_SIZE*vol, SEEK_SET);
		fread(rom_buff, 1, VOL_SIZE, f_rom);
		vol_idx = vol;
	}
}
 ...
 */
inline void ProcessBinary(uint8_t* dest, uint8_t* src){
    memcpy(dest + 0x4000, src , 0x4000);
    memcpy(dest, src + 0x4000, 0x4000);
}

void LoadRom(){
	size_t offset = 0;
	f_rom = fopen(nc1020_file_rom, "rb");
	while (offset < VOL_SIZE) {
		fread(temp_buff, 1, 0x8000, f_rom);
		ProcessBinary(rom_buff + offset, temp_buff);
		offset += 0x8000;
	}
	vol_idx = 0;
}
...
void LoadNor(){
	size_t offset = 0;
	FILE* file = fopen(nc1020_file_nor, "rb");
	while (offset < NOR_SIZE) {
		fread(temp_buff, 1, 0x8000, file);
		ProcessBinary(nor_buff + offset, temp_buff);
		offset += 0x8000;
	}
	fclose(file);
}
...
void SaveNor(){
	size_t offset = 0;
	FILE* file = fopen(nc1020_file_nor, "wb");
	while (offset < NOR_SIZE) {
		ProcessBinary(temp_buff, nor_buff + offset);
		fwrite(temp_buff, 1, 0x8000, file);
		offset += 0x8000;
	}
	fflush(file);
	fclose(file);
...
void Initialize(const char* path) {
	...
	for (size_t i=0; i<0x100; i++) {
		rom_volume[i] = rom_buff + (0x8000 * i);
